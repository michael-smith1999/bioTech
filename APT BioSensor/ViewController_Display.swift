...
//        dateOutput.text = UserDefaults.standard.string(forKey: tempString)
        print(queryVal)
        var rolls = UserDefaults.standard.object(forKey: queryVal + "rollValue") as? [String] ?? [""]
        print(rolls.count)
        var count = 0;
        for roll in rolls {
            if(count == 0){
        ...
        var pitches = UserDefaults.standard.object(forKey: queryVal + "pitchValue") as? [String] ?? [""]
        ...
        count = 0
        var yaws = UserDefaults.standard.object(forKey: queryVal + "yawValue") as? [String] ?? [""]
        for yaw in yaws {
            if(count == 0){
                yawOutput = yaw + "\n"
            }
